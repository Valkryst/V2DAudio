...
public class EffectSettingsTest {
    ...
    @Test
    public void testSetPan_withValueBelowMinimum() {
        final var settings = new EffectSettings();
        settings.setPan(-2);
        Assert.assertEquals(settings.getPan(), -1, 0.0);
    }

    @Test
    public void testSetPan_withValuesInRange() {
        final var settings = new EffectSettings();
        for (double i = -1 ; i < 1 ; i += 0.1) {
            settings.setPan(i);
            Assert.assertEquals(settings.getPan(), i, 0.0);
        }
    }

    @Test
    public void testSetPan_withValueAboveMaximum() {
        final var settings = new EffectSettings();
        settings.setPan(2);
        Assert.assertEquals(settings.getPan(), 1, 0.0);
    }
